...
{
	[SerializeField]
	private ZombieTransition _zombieTransition;
	...
	[SerializeField]
	private GameObject _borderToDisable;

	[SerializeField]
	private GameObject _borderToEnable;
	...
	[SerializeField]
	private FailZombie _failZombiePrefab;

	[SerializeField]
	private ZombieMason _masonZombie;

	[SerializeField]
	private Transform _masonBitePoint;
	...
	private IEnumerator FailRoutine()
	{
		...
		WaitForSeconds waitSpawnDelay = new WaitForSeconds(0.3f);
		for (int i = 0; i < 15; i++)
		{
			Vector3 position = Vector3.Lerp(_zombieSpawnPoints[0].position, _zombieSpawnPoints[1].position, Random.value);
			FailZombie zombie = Object.Instantiate(_failZombiePrefab, position, Quaternion.identity);
			if (i == 0)
			{
