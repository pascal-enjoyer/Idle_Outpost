...
namespace Factories
{
	public class PlankFactory : MonoBehaviour
	{
		...
		[SerializeField]
		private Transform _logTarget;

		[SerializeField]
		private Transform _plankSpawnPoint;
		...
		private readonly Queue<Resource> _logs = new Queue<Resource>();
		...
		private void Update()
		{
			if (!(Time.time - _pickup.LastPickupTime < _spawnDelayAfterPickup) && !(Time.time - _lastSpawnTime < 0.07f) && _upgraded && _stack.HasSpace)
			{
				SpawnPlank();
			}
		}
