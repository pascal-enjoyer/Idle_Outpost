...
namespace Characters
{
	...
	{
		[SerializeField]
		private MasonAnimator _animator;

		[SerializeField]
		private float _fireDelay = 0.5f;

		[SerializeField]
		private float _sqrFireRange = 2f;
		...
		private void Update()
		{
			if (!_stopped && !(Time.time - _lastTimeShoot < _fireDelay) && _storage.HasResource(ResourceType.MasonBullet) && _zombieManager.Alive.Count != 0 && TryGetRandomZombieInRange(out var target))
			{
		...
		private bool TryGetRandomZombieInRange(out Zombie target)
		{
			...
			{
				float sqrMagnitude = Vector2.SqrMagnitude(zombie.transform.position - base.transform.position);
