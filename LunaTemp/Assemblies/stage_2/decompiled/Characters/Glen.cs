...
namespace Characters
{
	public class Glen : MonoBehaviour
	{
		...
		[SerializeField]
		private GlenAnimator _animator;
		...
		[SerializeField]
		private ResourcePool _bulletPool;

		[SerializeField]
		private Transform _bulletPickupPoint;

		[SerializeField]
		private Transform _bulletSpawnPoint;

		[SerializeField]
		private Transform _bulletHoldingPoint;
		...
		private void PickUpBullet()
		{
			if (_inTransition)
			{
				...
				DOTween.Sequence().Append(_bullet.transform.DOScale(Vector3.one, _waitTime * 0.95f)).Join(_bullet.transform.DORotateQuaternion(_bulletHoldingPoint.rotation, _waitTime * 0.95f))
		...
		private void DropBullet()
		{
			if (_inTransition)
			{
				...
				_inTransition = false;
				_animator.Idle();
				DOVirtual.DelayedCall(_waitTime, delegate
				{
					SwitchState(GlenState.MovingToWarehouse);
				}, false);
			}
		...
		{
			base.transform.position = Vector3.MoveTowards(base.transform.position, position, _moveSpeed * Time.deltaTime);
			return base.transform.position == position;
		}

		private void PlayUpgradeEffect()
		{
			SpriteVfxService.Instance.Spawn(SpriteVfxType.Flash, base.transform.position + 0.5f * Vector3.up);
			DOTween.Sequence().Append(_upgradeMaterial.DOFade(1f, 1f)).Append(_upgradeMaterial.DOFade(0f, 1f));
		}
