...
namespace Trucks
{
	public class Truck : MonoBehaviour
	{
		...
		private void Update()
		{
			if (_waitingLoad && !_plankStack.IsEmpty && _colorZone.IsActive && !(Time.time - _lastTakeTime < _delay) && !(Time.time - _plankStack.LastTimeAdded < _pickupDelay))
			{
		...
		{
			base.transform.DORotateQuaternion(rotation, 0.4f);
			yield return base.transform.DOMove(position, 10f).SetEase(Ease.Linear).SetSpeedBased(true)
				.WaitForCompletion();
		}
		...
		{
			_waitingLoad = true;
			yield return new WaitUntil(() => !_waitingLoad);
