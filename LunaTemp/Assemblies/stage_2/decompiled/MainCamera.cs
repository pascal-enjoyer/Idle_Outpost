...
public class MainCamera : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		_offset = base.transform.position - _defaultTarget.position;
	}
	...
	private void LateUpdate()
	{
		...
		if (_isTransitioning)
		{
			...
			base.transform.position = Vector3.Lerp(_transitionStartPosition, targetPosition, _flyCurve.Evaluate(_transitionTime / _flyTime));
	...
	private IEnumerator ShowRoutine(Transform target, float time)
	{
		...
		_transitionTime = 0f;
		_transitionStartPosition = base.transform.position;
