...
namespace Conveyors
{
	...
	{
		[SerializeField]
		private bool _autoStarkWork;

		[SerializeField]
		private ConveyorItem _prefab;
		...
		{
			ConveyorItem item = Object.Instantiate(_prefab, base.transform, true);
		...
		private void Warmup()
		{
			...
			for (int i = 1; i < _initialAmount + 1; i++)
			{
				float distanceFromStart = distanceBetween * (float)i;
