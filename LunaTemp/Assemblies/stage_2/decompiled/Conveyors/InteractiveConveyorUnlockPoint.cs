...
namespace Conveyors
{
	public class InteractiveConveyorUnlockPoint : InteractiveConveyorPoint
	{
		...
		[SerializeField]
		private MaskedFillBar _fillBar;
		...
		public override void ProcessBullet(Bullet bullet)
		{
			...
			_currentCost--;
			bullet.Release();
			_fillBar.Render(1f - (float)_currentCost / (float)_cost);
			...
			DOTween.Sequence(target).Append(target.DOScale(scale * 0.8f, 0.2f)).Append(target.DOScale(scale, 0.2f))
		...
		private IEnumerator Unlock()
		{
			...
			yield return _toUnlock.transform.DoPopUp(0.3f, 1.1f, null, Vector3.one, true).WaitForCompletion();
