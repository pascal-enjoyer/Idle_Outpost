...
{
    [SerializeField] private ConveyorSelector _conveyorSelector;
    [SerializeField] private GameEnd _gameEnd;
    [SerializeField] private Storage _storage;
    [SerializeField] private ResourcePool _bulletsPool;
    [SerializeField] private ZombieManager _zombieManager;
    [SerializeField] private InteractiveConveyorUnlockPoint _unlockPoint;
    [SerializeField] private InteractiveConveyor _closedConveyor;
    [SerializeField] private InteractiveConveyor[] _conveyors;
    ...
    private void HandleBottomConveyorStep()
    {
        ...
        else
        {
            if (!_storage.HasResource(ResourceType.MasonBullet)
                && !_storage.HasResource(ResourceType.WarehouseBullet)
                && _bulletsPool.CountActive == 0
                && _zombieManager.Alive.Count > 0)
            {
                Fail();
            }
        }
    ...
    private void OnBulletReceived(InteractiveConveyor conveyor)
    {
        if (conveyor == _closedConveyor)
            ...
            _bulletsReceivedInOpenedConveyors += 1;
    }
