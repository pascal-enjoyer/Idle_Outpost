...
namespace Progression
{
    public class BuyZone : GameTrigger
    {
        ...
        [SerializeField] private ShaderFillBar _fillBar;
        [SerializeField] private TMP_Text _priceText;
        [SerializeField] private float _flyDelay;
        [SerializeField] private float _flyDuration;
        [SerializeField] private Transform _moneyReceivePoint;
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.TryGetComponent(out HeroStackManager stackManager))
            {
        ...
        {
            var items = new List<MoneyResource>();
            int price = Price;
            var itemsAmount = 0;

            while (price > 0 && !stack.IsEmpty)
            {
                var item = (MoneyResource)stack.RemoveItem(true);
                items.Add(item);
        ...
        private IEnumerator ThrowCoinArc(MoneyResource resource, Vector3 targetPos, float duration, bool isLast)
        {
            ...
            while (elapsed < duration)
            {
                ...
                float flipAngle = Mathf.Lerp(0, 180f, t);
                itemTransform.rotation = Quaternion.Euler(flipAngle, 0, 0);
            ...
            _money.Withdraw(MoneyResource.Price, 0.5f);
            Price -= MoneyResource.Price;
            DOVirtual.Int(Price + MoneyResource.Price, Price, 0.5f, UpdateVisuals);
            resource.Release();
            if (IsOpened && isLast)
            {
                IsOpened = true;
                ...
                InvokeTriggered();
                yield return transform.DoPopDown().WaitForCompletion();
        ...
        private void UpdateVisuals(int value)
        {
            ...
            _fillBar.SetProgress(1f - (float)value / _startPrice);
        }
    }
}