...
namespace Hero
{
    public class HeroMovement : MonoBehaviour
    {
        ...
        [SerializeField] private float _wheelRotationMultiplier;
        ...
        private void FixedUpdate()
        {
            ...
            Vector3 moveDirection =
                Vector3.ProjectOnPlane(_camera.forward, Vector3.up).normalized * _joystick.input.y +
                _camera.right * _joystick.input.x;

            bool moved = moveDirection != Vector3.zero;

            //_animator.SetRunMultiplier(runMultiplier);
            //_animator.SetForwardValue(Vector3.Dot(moveDirection, _animator.transform.forward));
            //_animator.SetRightValue(Vector3.Dot(moveDirection, _animator.transform.right));

            /*if (moved && !_movedLastFrame)
                _animator.StartRun();
            else if (!moved && _movedLastFrame)
                _animator.StopRun();*/
