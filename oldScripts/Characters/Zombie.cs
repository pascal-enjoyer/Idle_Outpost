...
namespace Characters
{
    public class Zombie : MonoBehaviour
    {
        ...
        [SerializeField] private Transform _checkSpacePoint;
        [SerializeField] private LayerMask _zombieLayerMask;
        [SerializeField] private float _checkSpaceRadius = 0.5f;
        ...
        private float SqrDistanceToTarget =>
            Vector3.SqrMagnitude(_target - transform.position);
        ...
        public void Tick()
        {
            _renderer.sortingOrder = -(int)(transform.position.y * 100 + 550);
        ...
        private bool HasSpace()
        {
            ...
            {
                if (_hits[i] != _collider
                    && _hits[i].TryGetComponent(out Zombie zombie)
                    && (zombie._isWaiting || zombie.IsAttacking)
                    && zombie.SqrDistanceToTarget < SqrDistanceToTarget)
                {
                    return false;
                }
            }
        ...
        {
            SpriteVfxService.Instance.Spawn(SpriteVfxType.ZombieBlood, transform.position + 0.82f * Vector3.up);
            SpriteVfxService.Instance.Spawn(SpriteVfxType.Sparks, transform.position + 0.82f * Vector3.up);
            IsAttacking = false;
            _isWalking = false;
            _animator.Death();
