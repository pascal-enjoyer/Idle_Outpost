...
namespace Characters
{
    public class ZombieManager : MonoBehaviour
    {
        ...
        [SerializeField] private int _maximumOnScreen = 100;
        [SerializeField] private AudioClip _zombieBiteSound;
        ...
        public HashSet<Zombie> Alive { get; } = new();
        ...
        private void SetZombieTarget(Zombie zombie)
        {
            ...
            if (zombieY >= minY && zombieY <= maxY)
            {
                ...
                targetY = zombieY + dir * offset;
            }
            else
            {
                float offset = zombieY < minY ? Random.Range(0f, 5f) : Random.Range(0, -5f);
                float y = zombieY < minY ? minY : maxY;
                targetY = y + offset;
            }

            var targetPosition = new Vector3(targetX, targetY, 0f);
        ...
        {
            Zombie zombie = Instantiate(_prefab, transform);
